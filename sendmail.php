 ...
 */
function getTopText () {
    $message = "You received a new message sent via the contact form on grundimagineering.fi.";
    $message = strlen($message) > 0 ? $message : "You received a new message sent via the contact form on grundimagineering.fi.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "grundimagineering.fi"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 115.1.31",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "grundimagineering.fi") {
...
function processMail() {
    ...
    if (empty($response)) {
        ...
        $contactFormEmails = array_map("toLower", json_decode('[]'));
        if (
